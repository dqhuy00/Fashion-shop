	<!-- Shoping Cart -->
	<?php

	$totalPrice = array_reduce($cart, function ($total, $cart) {
		return ($cart['price'] * $cart['quantity']);
	})
	?>
	<div class="bg0 p-t-75 p-b-85">
		<div class="container">
			<div class="row">
				<div class="col-lg-10 col-xl-7 m-lr-auto m-b-50">
					<div class="m-l-25 m-r--38 m-lr-0-xl">
						<div class="wrap-table-shopping-cart">
							<table class="table-shopping-cart">
								<thead>
									<tr class="table_head">
										<th class="column-1">tên</th>
										<th class="column-2"></th>
										<th class="column-3">giá</th>
										<th class="column-4">số lượng</th>
										<th class="column-5">tổng giá</th>
									</tr>
								</thead>
								<tbody>
									<?php foreach ($cart as $value) : ?>
										<tr class="table_row">
											<td class="column-1">
												<a href="?controller=shop&action=delete-cart&id=<?= $value['id']  ?>">
													<div class="how-itemcart1">
														<img src="<?= $value['images']  ?>" alt="IMG">
													</div>
												</a>
											</td>
											<td class="column-2"><?= $value['name'] . ' ' . join('-', array_column($value['attr'], 'name')) ?></td>
											<td class="column-3"><?= number_format($value['price']) ?> đ</td>
											<td class="column-4">
												<div class="wrap-num-product flex-w m-l-auto m-r-0">
													<div class="btn-num-product-down cl8 hov-btn3 trans-04 flex-c-m">
														<i class="fs-16 zmdi zmdi-minus"></i>
													</div>

													<input class="mtext-104 cl3 txt-center num-product" type="number" name="num-product1" value="<?= $value['quantity']  ?>">

													<div class="btn-num-product-up cl8 hov-btn3 trans-04 flex-c-m">
														<i class="fs-16 zmdi zmdi-plus"></i>
													</div>
												</div>
											</td>
											<td class="column-5"><?= number_format($value['quantity']  * $value['price']) ?></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
						</div>

						<?php if (!empty($cart) && count($cart) > 0) : ?>
							<div class="flex-w flex-sb-m bor15 p-t-18 p-b-15 p-lr-40 p-lr-15-sm">
								<div class="flex-w flex-m m-r-20 m-tb-5">
									<input class="stext-104 cl2 plh4 size-117 bor13 p-lr-20 m-r-10 m-tb-5" type="text" name="coupon" placeholder="Coupon Code">

									<div class="flex-c-m stext-101 cl2 size-118 bg8 bor13 hov-btn3 p-lr-15 trans-04 pointer m-tb-5">
										Ap dụng phí giảm giá
									</div>
								</div>

								<div class="flex-c-m stext-101 cl2 size-119 bg8 bor13 hov-btn3 p-lr-15 trans-04 pointer m-tb-10">
									thêm sản phẩm
								</div>
							</div>
						<?php endif ?>
					</div>
				</div>

				<div class="col-sm-10 col-lg-7 col-xl-5 m-lr-auto m-b-50">
					<div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
						<h4 class="mtext-109 cl2 p-b-30">
							Cart Totals
						</h4>

						<div class="flex-w flex-t bor12 p-b-13">
							<div class="size-208">
								<span class="stext-110 cl2">
									phụ thu:
								</span>
							</div>

							<div class="size-209">
								<span class="mtext-110 cl2">
									0
								</span>
							</div>
						</div>

						<div class="flex-w flex-t p-t-27 p-b-33">
							<div class="size-208">
								<span class="mtext-101 cl2">
									tổng giá:
								</span>
							</div>

							<div class="size-209 p-t-1">
								<span class="mtext-110 cl2 total-price">
									<?= number_format($totalPrice) ?>
								</span>
							</div>
						</div>
						<a href="?controller=checkout" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer">
							thanh toán
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>